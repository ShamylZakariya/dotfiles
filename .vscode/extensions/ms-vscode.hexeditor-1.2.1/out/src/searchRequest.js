...
class SearchRequest {
     ...
     */
    normalHexSearch(query, documentIndex, results, onComplete) {
     ...
     * @param {(value: SearchResults) => void} onComplete Callback which is called when the function is completed
     */
    regexTextSearch(query, caseSensitive, results, documentString, onComplete) {
        const searchStart = Date.now();
     ...
     */
    normalTextSearch(query, caseSensitive, documentIndex, results, onComplete) {
        const searchStart = Date.now();
        // We compare the query to every spot in the file finding matches
        for (; documentIndex < this._documentDataWithEdits.length; documentIndex++) {
            const matchOffsets = [];
