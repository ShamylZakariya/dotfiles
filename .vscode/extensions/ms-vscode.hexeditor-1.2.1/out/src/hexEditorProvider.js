...
const hexDocument_1 = require("./hexDocument");
const dispose_1 = require("./dispose");
const webViewCollection_1 = require("./webViewCollection");
const path = require("path");
const util_1 = require("./util");
class HexEditorProvider {
    constructor(_context, _telemetryReporter) {
        this._context = _context;
        this._telemetryReporter = _telemetryReporter;
        this.webviews = new webViewCollection_1.WebviewCollection();
        this._onDidChangeCustomDocument = new vscode.EventEmitter();
        this.onDidChangeCustomDocument = this._onDidChangeCustomDocument.event;
        this._requestId = 1;
        this._callbacks = new Map();
    }
    static register(context, telemetryReporter) {
        return vscode.window.registerCustomEditorProvider(HexEditorProvider.viewType, new HexEditorProvider(context, telemetryReporter), {
            supportsMultipleEditorsPerDocument: false
        });
    }
    async openCustomDocument(uri, openContext, token) {
        const document = await hexDocument_1.HexDocument.create(uri, openContext.backupId, this._telemetryReporter);
    ...
    */
    getHtmlForWebview(webview) {
        ...
        const scriptUri = webview.asWebviewUri(vscode.Uri.file(path.join(this._context.extensionPath, "dist", "bundle.js")));
        const styleUri = webview.asWebviewUri(vscode.Uri.file(path.join(this._context.extensionPath, "dist", "hexEdit.css")));
        const codiconsUri = webview.asWebviewUri(vscode.Uri.file(path.join(this._context.extensionPath, "node_modules", "vscode-codicons", "dist", "codicon.css")));
        const codiconsFontUri = webview.asWebviewUri(vscode.Uri.file(path.join(this._context.extensionPath, "node_modules", "vscode-codicons", "dist", "codicon.ttf")));
    ...
    async onMessage(panel, document, message) {
        switch (message.type) {
            ...
            case "replace":
                ...
                const replaced = document.replace(message.body.query, message.body.offsets, message.body.preserveCase);
...
HexEditorProvider.viewType = "hexEditor.hexedit";
